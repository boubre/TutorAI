...
namespace GeometryTutorLib.AtomicRegionIdentifier
{
    ...
    public static class AtomicIdentifierMain
    {
        ...
        private static List<AtomicRegion> IdentifyPathological(List<Point> figurePoints, List<AtomicRegion> atoms, List<GeometryTutorLib.ConcreteAST.Circle> circles)
        {
            ...
            //
            List <AtomicRegion> ars = AcquireAtomicRegionsFromGraph(points, segments, arcs, circles);
            List <AtomicRegion> trueAtoms = new List<AtomicRegion>();
                                                                        ...
                                                                        List<Arc> arcs, List<GeometryTutorLib.ConcreteAST.Circle> circles)
        {
            ...
            foreach (Arc arc in arcs)
            {
                ...
                {
                    //if (graph.GetEdge(arc.collinear[p], arc.collinear[p + 1]) == null)
                    //{
                    ...
                    ////
                    //// To ensure proper acquisition of arcs, add the midpoint of the arc as a connector. That is, for each arc, 
                    //// we create:  endpt --- midpt --- endpt2
                    ////
                    //else
                    //{
                    //    Point midpt = arc.theCircle.Midpoint(arc.collinear[p], arc.collinear[p + 1]);


                    //    graph.AddNode(midpt);

                    //    graph.AddUndirectedEdge(arc.collinear[p], midpt,
                    //                            new GeometryTutorLib.ConcreteAST.Segment(arc.collinear[p], midpt).Length,
                    //                            GeometryTutorLib.Area_Based_Analyses.Atomizer.UndirectedPlanarGraph.EdgeType.REAL_DUAL);

                    //    graph.AddUndirectedEdge(midpt, arc.collinear[p + 1],
                    //                            new GeometryTutorLib.ConcreteAST.Segment(midpt, arc.collinear[p + 1]).Length,
                    //                            GeometryTutorLib.Area_Based_Analyses.Atomizer.UndirectedPlanarGraph.EdgeType.REAL_DUAL);
                    //}
                }
            }
