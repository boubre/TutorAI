    ...
    public class DrawingParser
    {
            ...
            for (int i = 0; i < segments.Length - 1; i++)
            {
                for (int j = i + 1; j < segments.Length; j++)
                {
                    ...
                    if (segments[i].Point1.X == intersection.X || segments[i].Point1.Y == intersection.Y ||
                        ...
                        segments[j].Point2.X == intersection.X || segments[j].Point2.Y == intersection.Y)
                        continue;

                    //Make sure it is on the lines
                    if (!(isInMiddle(segments[i].Point1, intersection, segments[i].Point2)
                        && isInMiddle(segments[j].Point1, intersection, segments[j].Point2))) 
                        continue;
        ...
        public void calculateTriangles(List<GroundedClause> clauses)
        {
            ...
            for (int i = 0; i < segments.Length - 2; i++)
            {
                ...
                {
                    if (segments[i].Equals(segments[j]) || segments[i].Slope == segments[j].Slope) continue;
                    else if (segments[i].Point1.Equals(segments[j].Point1))
                        for (int k = j + 1; k < segments.Length; k++)
                        {
                            if (segments[k].Equals(segments[i]) || segments[k].Equals(segments[j])) continue;
                            else if (segments[k].Slope == segments[i].Slope || segments[k].Slope == segments[j].Slope) continue;
